...
| Generics | type parameters in AST and resolved types | checker infers/substitutes concrete arguments | `emitter-monomorphize.do` discovers a fixed point; emitters output concrete forms, except generic methods on non-generic owners remain C++ member templates |
