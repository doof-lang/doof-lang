...
import { prepareRelease, checkSourceSnapshot, signingPreflight, windowsEndToEnd } from "./release"
...
function dispatch(arguments: string[]): Result<int, string> {
  try require(arguments.length >= 1, "usage: repository <root> <dev|build|test|release|windows-test> [version]")
  ...
  if arguments[1] != "release" && arguments[1] != "cache-test" && arguments[1] != "windows-test" { try require(arguments.length == 2, "Unexpected repository command arguments") }
