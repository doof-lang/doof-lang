...
export function testRepositoryStampIsolatedInputs(): none {
  ...
  try! write(path(source, "doof.json"), "{\"version\":\"0.2.0\"}")
  try! write(path(source, "tools/debugger/doof.json"), "{\"version\":\"0.2.0\"}")
  try! write(path(source, "src/version.do"), "export readonly compilerVersion = \"0.2.0-dev.unstamped\"\nexport readonly compilerVersionStamped = false\n")
  ...
  Assert.equal(try! jsonString(try! jsonFile(path(source, "doof.json")), "version"), "0.2.0")
