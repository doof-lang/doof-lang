...
doof::Result<std::shared_ptr<Compilation>, std::string> Compilation::fromJsonValue(const doof::JsonValue& _json, bool _lenient) {
    ...
    auto _field_emission = (doof::json_is_null(_iterator_emission->second) ? nullptr : doof::json_decode_value(::app_src_emitter_module_::ModuleGraphEmission::fromJsonValue(_iterator_emission->second, _lenient)));
    ...
    auto _field_diagnostics = [&]() { const auto* _array = doof::json_as_array(_iterator_diagnostics->second); auto _values = std::make_shared<std::vector<std::shared_ptr<::app_src_semantic_::Diagnostic>>>(); _values->reserve(_array->size()); for (const auto& _element : *_array) { _values->push_back(doof::json_decode_value(::app_src_semantic_::Diagnostic::fromJsonValue(_element, _lenient))); } return _values; }();
    ...
    if (auto _iterator_sourceFiles = _object->find("sourceFiles"); _iterator_sourceFiles != _object->end()) {
        ...
        _field_sourceFiles = [&]() { const auto* _array = doof::json_as_array(_iterator_sourceFiles->second); auto _values = std::make_shared<std::vector<std::shared_ptr<::app_src_semantic_::SourceFile>>>(); _values->reserve(_array->size()); for (const auto& _element : *_array) { _values->push_back(doof::json_decode_value(::app_src_semantic_::SourceFile::fromJsonValue(_element, _lenient))); } return _values; }();
    } else {
...
std::shared_ptr<Compilation> compileInternal(const std::shared_ptr<std::vector<std::shared_ptr<::app_src_semantic_::SourceFile>>>& sources, const std::string& entry, const doof::callback<doof::Result<std::shared_ptr<::app_src_semantic_::SourceFile>, std::shared_ptr<::app_src_semantic_::Diagnostic>>(std::string)>& loader, const std::shared_ptr<std::vector<std::shared_ptr<::app_src_emitter_names_::ModuleNamespaceMapping>>>& namespaceMappings, const std::string& entryMode, bool coverage, bool emit, const std::shared_ptr<std::vector<std::shared_ptr<::app_src_emitter_module_::ModuleEmissionCacheKey>>>& reusableModules, const std::string& emissionConfigurationFingerprint) {
    ...
    if (entryMode == std::string("wasm")) {
        auto _binding_value_13 = ::app_src_emitter_wasm_::emitWasmSupport(analysis, entry);
