...
doof::Result<std::shared_ptr<MacOSAppConfig>, std::string> MacOSAppConfig::fromJsonValue(const doof::JsonValue& _json, bool _lenient) {
    ...
    if (auto _iterator_resources = _object->find("resources"); _iterator_resources != _object->end()) {
        ...
        _field_resources = [&]() { const auto* _array = doof::json_as_array(_iterator_resources->second); auto _values = std::make_shared<std::vector<std::shared_ptr<MacOSAppResource>>>(); _values->reserve(_array->size()); for (const auto& _element : *_array) { _values->push_back(doof::json_decode_value(MacOSAppResource::fromJsonValue(_element, _lenient))); } return _values; }();
    } else {
    ...
    if (auto _iterator_embeddedLibraries = _object->find("embeddedLibraries"); _iterator_embeddedLibraries != _object->end()) {
        ...
        _field_embeddedLibraries = [&]() { const auto* _array = doof::json_as_array(_iterator_embeddedLibraries->second); auto _values = std::make_shared<std::vector<std::shared_ptr<MacOSEmbeddedLibrary>>>(); _values->reserve(_array->size()); for (const auto& _element : *_array) { _values->push_back(doof::json_decode_value(MacOSEmbeddedLibrary::fromJsonValue(_element, _lenient))); } return _values; }();
    } else {
