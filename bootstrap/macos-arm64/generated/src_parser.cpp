...
doof::Result<std::shared_ptr<Parser>, std::string> Parser::fromJsonValue(const doof::JsonValue& _json, bool _lenient) {
    ...
    if (auto _iterator_tokens = _object->find("tokens"); _iterator_tokens != _object->end()) {
        ...
        _field_tokens = [&]() { const auto* _array = doof::json_as_array(_iterator_tokens->second); auto _values = std::make_shared<std::vector<::app_src_lexer_::Token>>(); _values->reserve(_array->size()); for (const auto& _element : *_array) { _values->push_back(doof::json_decode_value(::app_src_lexer_::Token::fromJsonValue(_element, _lenient))); } return _values; }();
    } else {
        _field_tokens = std::make_shared<std::vector<::app_src_lexer_::Token>>(std::vector<::app_src_lexer_::Token>{});
    }
    ...
    if (auto _iterator_errorMessage = _object->find("errorMessage"); _iterator_errorMessage != _object->end()) {
        ...
        _field_errorMessage = std::string("");
    }
