...
void typeError(std::shared_ptr<::app_src_checker_state_::CheckerState> state, std::string message, ::app_src_ast_::SourceSpan span) {
    state->diagnostics->push_back(std::make_shared<::app_src_semantic_::Diagnostic>(std::string("error"), message, ::app_src_checker_validation_::checkerSemanticSpan(span), state->info->path));
}
