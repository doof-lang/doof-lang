...
namespace app_src_emitter_monomorphize_ {
    ...
    std::string interfaceInstantiationKey(const std::string& modulePath, const std::string& name, const std::shared_ptr<std::vector<doof_header_type_11>>& typeArgs);
    std::string concreteName(const std::string& name, const std::shared_ptr<std::vector<doof_header_type_11>>& typeArgs);
}
...
namespace app_src_emitter_types_ {
    ...
    std::string emitWeakType(const doof_header_type_12& inner, const std::string& currentModulePath);
    std::string emitParameterType(const doof_header_type_12& resolvedType, const std::string& currentModulePath);
    std::string borrowParameterType(const doof_header_type_12& resolvedType, const std::string& emittedType);
    bool canBorrowParameter(const doof_header_type_12& resolvedType);
    bool requiresParameterValueSemantics(const doof_header_type_12& resolvedType);
    std::string emitMetadataInnerType(const doof_header_type_12& owner, const std::string& currentModulePath);
    ...
    bool usesNullableSingleValueRepresentation(const doof_header_type_12& type_);
    ...
    bool usesNaturalNullableMember(const doof_header_type_12& member);
    std::shared_ptr<std::vector<doof_header_type_12>> flattenUnionMembers(const std::shared_ptr<std::vector<doof_header_type_12>>& types);
