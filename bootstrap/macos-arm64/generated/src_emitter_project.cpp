...
doof::Result<std::shared_ptr<ProjectEmission>, std::string> ProjectEmission::fromJsonValue(const doof::JsonValue& _json, bool _lenient) {
    ...
    if (auto _iterator_modules = _object->find("modules"); _iterator_modules != _object->end()) {
        ...
        _field_modules = [&]() { const auto* _array = doof::json_as_array(_iterator_modules->second); auto _values = std::make_shared<std::vector<std::shared_ptr<::app_src_emitter_module_::ModuleEmission>>>(); _values->reserve(_array->size()); for (const auto& _element : *_array) { _values->push_back(doof::json_decode_value(::app_src_emitter_module_::ModuleEmission::fromJsonValue(_element, _lenient))); } return _values; }();
    } else {
    ...
    if (auto _iterator_supportFiles = _object->find("supportFiles"); _iterator_supportFiles != _object->end()) {
        ...
        _field_supportFiles = [&]() { const auto* _array = doof::json_as_array(_iterator_supportFiles->second); auto _values = std::make_shared<std::vector<std::shared_ptr<ProjectSupportFile>>>(); _values->reserve(_array->size()); for (const auto& _element : *_array) { _values->push_back(doof::json_decode_value(ProjectSupportFile::fromJsonValue(_element, _lenient))); } return _values; }();
    } else {
    ...
    if (auto _iterator_nativeCopies = _object->find("nativeCopies"); _iterator_nativeCopies != _object->end()) {
        ...
        _field_nativeCopies = [&]() { const auto* _array = doof::json_as_array(_iterator_nativeCopies->second); auto _values = std::make_shared<std::vector<std::shared_ptr<ProjectNativeCopy>>>(); _values->reserve(_array->size()); for (const auto& _element : *_array) { _values->push_back(doof::json_decode_value(ProjectNativeCopy::fromJsonValue(_element, _lenient))); } return _values; }();
    } else {
    ...
    if (auto _iterator_nativeBuild = _object->find("nativeBuild"); _iterator_nativeBuild != _object->end()) {
        ...
        _field_nativeBuild = doof::json_decode_value(::app_src_package_manifest_::NativeBuildPlan::fromJsonValue(_iterator_nativeBuild->second, _lenient));
    } else {
