...
std::shared_ptr<ProjectEmission> planProjectEmission(const std::shared_ptr<::app_src_emitter_module_::ModuleGraphEmission>& graph, const std::shared_ptr<std::vector<std::shared_ptr<NativePackageInput>>>& packages) {
    ...
    if (graph->wasmSupportSource != std::string("")) {
        ...
        static_cast<void>((project->wasmExportNames = graph->wasmExportNames));
    }
    const auto& _iterable_2 = orderedNativePackages(packages);
...
std::shared_ptr<std::vector<std::shared_ptr<NativePackageInput>>> orderedNativePackages(const std::shared_ptr<std::vector<std::shared_ptr<NativePackageInput>>>& packages) {
    std::shared_ptr<std::vector<std::shared_ptr<NativePackageInput>>> result = std::make_shared<std::vector<std::shared_ptr<NativePackageInput>>>(std::vector<std::shared_ptr<NativePackageInput>>{});
    const auto& _iterable_4 = packages;
    for (const auto& package_ : *_iterable_4) {
        result->push_back(package_);
        auto index = (static_cast<int32_t>((result)->size()) - 1);
        const auto key = nativePackageOrderKey(package_);
        while ((index > 0) && (nativePackageOrderKey(doof::array_at(result, (index - 1), "src/emitter-project", 65)) > key)) {
            static_cast<void>((doof::array_at(result, index, "src/emitter-project", 66) = doof::array_at(result, (index - 1), "src/emitter-project", 66)));
            static_cast<void>((index -= 1));
        }
        static_cast<void>((doof::array_at(result, index, "src/emitter-project", 69) = package_));
    }
    return result;
}
std::string nativePackageOrderKey(const std::shared_ptr<NativePackageInput>& package_) {
    return ((package_->outputRoot == std::string("")) ? std::string("") : package_->logicalPrefix);
}
void planPackageSupportFiles(const std::shared_ptr<ProjectEmission>& project, const std::shared_ptr<::app_src_emitter_module_::ModuleGraphEmission>& graph, const std::shared_ptr<NativePackageInput>& package_) {
    const auto& _iterable_6 = graph->modules;
...
void planPackageNativeBuild(const std::shared_ptr<ProjectEmission>& project, const std::shared_ptr<NativePackageInput>& package_) {
    ...
    for (const auto& includePath : *_iterable_8) {
        ...
        static_cast<void>((copiedAny = true));
    }
    const auto& _iterable_10 = native->sourceFiles;
    ...
    const auto& _iterable_12 = native->libraryPaths;
    for (const auto& libraryPath : *_iterable_12) {
    ...
    const auto& _iterable_14 = native->extraCopyPaths;
    for (const auto& extraCopyPath : *_iterable_14) {
...
std::string addNativeCopy(const std::shared_ptr<std::vector<std::shared_ptr<ProjectNativeCopy>>>& copies, const std::shared_ptr<NativePackageInput>& package_, const std::string& sourcePath) {
    ...
    const auto& _iterable_16 = copies;
...
std::string projectParentPath(const std::string& path) {
    ...
    while (index < static_cast<int32_t>(path.size())) {
        if (doof::string_at(path, index, "src/emitter-project", 190) == U'\u002F') {
...
void appendUnique(const std::shared_ptr<std::vector<std::string>>& target, const std::string& value) {
    const auto& _iterable_22 = target;
