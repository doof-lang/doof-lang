...
namespace app_src_ast_ {
    ...
    __type2 resolvedType = std::monostate{};
    SourceSpan span;
    TypePattern(std::string kind, std::string name, TypeAnnotation type_, __type2 resolvedType, SourceSpan span) : kind(kind), name(name), type_(type_), resolvedType(resolvedType), span(span) {}
};
}

namespace app_src_emitter_types_ {
    std::string emitResultPayloadType(const __type3& resolvedType, const std::string& currentModulePath = std::string(""));
    std::string emitType(const __type3& resolvedType, const std::string& currentModulePath = std::string(""));
    bool usesVariantRepresentation(const __type3& type_);
    bool usesNullableSingleValueRepresentation(const __type3& type_);
}

namespace app_src_emitter_case_pattern_ {
    std::shared_ptr<CaseTypePatternEmission> emitCaseTypePattern(const std::shared_ptr<::app_src_ast_::TypePattern>& pattern, const __type4& subjectType, const std::string& subject, const std::string& bindingName, const std::string& currentModulePath);
    ...
    std::shared_ptr<CaseTypePatternEmission> emitJsonValuePattern(const __type4& patternType, const std::string& subject, const std::string& bindingName);
}
