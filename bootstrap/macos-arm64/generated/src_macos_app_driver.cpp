...
void ensureDirectory(const std::string& path) {
    ...
    static_cast<void>([&]() -> std::monostate { auto _try_value = ::doof_fs::mkdir(path); if (doof::is_failure(_try_value)) doof::panic_at("src/macos-app-driver", 45, std::string("try! failed"));  return {}; }());
}
...
void removeTree(const std::string& path) {
    ...
    static_cast<void>([&]() -> std::monostate { auto _try_value = ::doof_fs::remove(path); if (doof::is_failure(_try_value)) doof::panic_at("src/macos-app-driver", 65, std::string("try! failed"));  return {}; }());
}
std::string globBaseDirectory(const std::string& pattern) {
    ...
    while ((slash > 0) && (doof::string_substring(pattern, (slash - 1), slash) != std::string("/"))) {
        static_cast<void>((slash = (slash - 1)));
    }
...
void materializeMacOSResources(const std::shared_ptr<::app_src_macos_app_::MacOSAppConfig>& config, const std::string& resourcesDirectory) {
    ...
    const auto& _iterable_12 = config->resources;
    for (const auto& resource : *_iterable_12) {
        ...
        const auto prefix = (doof::string_endsWith(baseDirectory, std::string("/")) ? baseDirectory : (baseDirectory + std::string("/")));
        const auto& _iterable_10 = files;
        for (const auto& sourcePath : *_iterable_10) {
...
doof::Result<std::string, std::string> resolveEmbeddedLibrary(const std::string& name, const std::shared_ptr<std::vector<std::string>>& libraryPaths, const std::string& buildDirectory) {
    ...
    const auto& _iterable_18 = libraryPaths;
    for (const auto& libraryPath : *_iterable_18) {
        ...
        const auto& _iterable_16 = candidates;
        for (const auto& candidate : *_iterable_16) {
...
doof::Result<void, std::string> rewriteEmbeddedDependencies(const std::string& codePath, const std::shared_ptr<std::vector<std::shared_ptr<EmbeddedCode>>>& embedded) {
    ...
    const auto& _iterable_36 = dependencies;
    for (const auto& dependency : *_iterable_36) {
        ...
        if (dependency != target->bundleReference) {
            auto _try_value_34 = runRequiredCommand(std::string("install_name_tool"), std::make_shared<std::vector<std::string>>(std::vector<std::string>{std::string("-change"), dependency, target->bundleReference, codePath}), std::string("rewriting embedded library dependency"));
...
doof::Result<void, std::string> embedMacOSLibraries(const std::string& executablePath, const std::shared_ptr<::app_src_macos_app_::MacOSAppConfig>& config, const std::shared_ptr<std::vector<std::string>>& libraryPaths, const std::string& buildDirectory, const std::string& contentsDirectory) {
    ...
    for (const auto& entry : *_iterable_42) {
        ...
        if (sourcePath == std::string("")) {
            auto _try_value_38 = resolveEmbeddedLibrary(entry->library, libraryPaths, buildDirectory);
            ...
            const auto resolvedSourcePath = doof::success_value(_try_value_38);
            static_cast<void>((sourcePath = resolvedSourcePath));
        }
        auto _try_value_39 = planEmbeddedCode(sourcePath, frameworksDirectory);
        ...
        const auto code = doof::success_value(_try_value_39);
    ...
    const auto& _iterable_48 = embedded;
    for (const auto& code : *_iterable_48) {
        auto _try_value_44 = rewriteEmbeddedDependencies(code->bundledPath, embedded);
        ...
        auto _try_value_45 = runRequiredCommand(std::string("install_name_tool"), std::make_shared<std::vector<std::string>>(std::vector<std::string>{std::string("-id"), code->bundleReference, code->bundledPath}), std::string("setting embedded library install name"));
        ...
        auto _try_value_46 = ensureMachORPath(code->bundledPath, std::string("@loader_path"));
...
doof::Result<std::string, std::string> assembleMacOSApp(const std::string& buildDirectory, const std::string& executablePath, const std::shared_ptr<::app_src_macos_app_::MacOSAppConfig>& config, const std::shared_ptr<std::vector<std::string>>& libraryPaths) {
    ...
    static_cast<void>([&]() -> std::monostate { auto _try_value = ::doof_fs::writeText(outputPath(contentsDirectory, std::string("Info.plist")), ::app_src_macos_app_::renderMacOSInfoPlist(config)); if (doof::is_failure(_try_value)) doof::panic_at("src/macos-app-driver", 374, std::string("try! failed"));  return {}; }());
    static_cast<void>([&]() -> std::monostate { auto _try_value = ::doof_fs::writeText(outputPath(contentsDirectory, std::string("PkgInfo")), std::string("APPL\?\?\?\?")); if (doof::is_failure(_try_value)) doof::panic_at("src/macos-app-driver", 375, std::string("try! failed"));  return {}; }());
...
doof::Result<std::string, std::string> developerIdIdentity(const std::string& configured) {
    ...
    const auto& _iterable_61 = doof::string_split(commandOutput, std::string("\n"));
    for (const auto& line : *_iterable_61) {
        auto start = -1;
        for (int32_t index = 0; index < ((static_cast<int32_t>(line.size()) - static_cast<int32_t>(marker.size())) + 1); ++index) {
            if (doof::string_substring(line, index, (index + static_cast<int32_t>(marker.size()))) == marker) {
                static_cast<void>((start = (index + 1)));
                break;
            }
        }
        if (start < 0) {
...
doof::Result<void, std::string> signAndArchiveMacOSApp(const std::string& appPath, const std::string& archivePath, const std::shared_ptr<::app_src_macos_app_::MacOSPackageConfig>& config, const std::string& buildDirectory) {
    ...
    if (config->signing != std::string("ad-hoc")) {
        ...
        static_cast<void>((identity = resolvedIdentity));
    }
    ...
    if (::doof_fs::exists(archivePath)) {
        static_cast<void>([&]() -> std::monostate { auto _try_value = ::doof_fs::remove(archivePath); if (doof::is_failure(_try_value)) doof::panic_at("src/macos-app-driver", 455, std::string("try! failed"));  return {}; }());
    }
