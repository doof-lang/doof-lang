    ...
    auto armType = std::string("");
    {
        ...
        if (std::holds_alternative<std::shared_ptr<::app_src_ast_::NamedType>>(_case_subject)) {
            ...
            if (named->name == std::string("Success")) {
                (armType = ((std::string("doof::Success<") + ::app_src_emitter_types_::emitType(result->valueType, currentModulePath)) + std::string(">")));
            }
            if (named->name == std::string("Failure")) {
                (armType = ((std::string("doof::Failure<") + ::app_src_emitter_types_::emitType(result->errorType, currentModulePath)) + std::string(">")));
            }
    }
    else {
    }
