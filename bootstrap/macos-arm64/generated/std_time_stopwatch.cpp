...
doof::Result<std::shared_ptr<TimerStats>, std::string> TimerStats::fromJsonValue(const doof::JsonValue& _json, bool _lenient) {
    ...
    auto _field_total = doof::json_decode_value(::std_::time::duration::Duration::fromJsonValue(_iterator_total->second, _lenient));
    ...
    auto _field_mean = doof::json_decode_value(::std_::time::duration::Duration::fromJsonValue(_iterator_mean->second, _lenient));
    ...
    auto _field_min = doof::json_decode_value(::std_::time::duration::Duration::fromJsonValue(_iterator_min->second, _lenient));
    ...
    auto _field_max = doof::json_decode_value(::std_::time::duration::Duration::fromJsonValue(_iterator_max->second, _lenient));
    ...
    auto _field_p95 = doof::json_decode_value(::std_::time::duration::Duration::fromJsonValue(_iterator_p95->second, _lenient));
...
doof::Result<std::shared_ptr<TimerSummary>, std::string> TimerSummary::fromJsonValue(const doof::JsonValue& _json, bool _lenient) {
    ...
    auto _field_entries = [&]() { const auto* _array = doof::json_as_array(_iterator_entries->second); auto _values = std::make_shared<std::vector<std::shared_ptr<TimerStats>>>(); _values->reserve(_array->size()); for (const auto& _element : *_array) { _values->push_back(doof::json_decode_value(TimerStats::fromJsonValue(_element, _lenient))); } return _values; }();
...
doof::Result<std::shared_ptr<StopwatchSpan>, std::string> StopwatchSpan::fromJsonValue(const doof::JsonValue& _json, bool _lenient) {
    ...
    auto _field_stopwatch = doof::json_decode_value(Stopwatch::fromJsonValue(_iterator_stopwatch->second, _lenient));
    auto _iterator_name = _object->find("name");
    if (_iterator_name == _object->end()) { return doof::Failure<std::string>{"Missing required field \"name\""}; }
        if (!((_lenient ? doof::json_is_lenient_string(_iterator_name->second) : doof::json_is_string(_iterator_name->second)))) { return doof::Failure<std::string>{"Field \"name\" expected string but got " + std::string(doof::json_type_name(_iterator_name->second))}; }
    auto _field_name = (_lenient ? doof::json_as_string_lenient(_iterator_name->second) : doof::json_as_string(_iterator_name->second));
    ...
    auto _field_startedAt = doof::json_decode_value(::std_::time::temporal::Instant::fromJsonValue(_iterator_startedAt->second, _lenient));
    ...
    if (auto _iterator_finishedDuration = _object->find("finishedDuration"); _iterator_finishedDuration != _object->end()) {
        ...
        _field_finishedDuration = (doof::json_is_null(_iterator_finishedDuration->second) ? nullptr : doof::json_decode_value(::std_::time::duration::Duration::fromJsonValue(_iterator_finishedDuration->second, _lenient)));
    } else {
