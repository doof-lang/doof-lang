...
doof::JsonObject ProjectSpec::toJsonObject() const {
    ...
    (*_json)["explicitEntry"] = doof::json_value(this->explicitEntry);
...
doof::Result<std::shared_ptr<ProjectSpec>, std::string> ProjectSpec::fromJsonValue(const doof::JsonValue& _json, bool _lenient) {
    ...
    std::optional<bool> _field_explicitEntry;
    if (auto _iterator_explicitEntry = _object->find("explicitEntry"); _iterator_explicitEntry != _object->end()) {
            if (!((_lenient ? doof::json_is_lenient_boolean(_iterator_explicitEntry->second) : doof::json_is_boolean(_iterator_explicitEntry->second)))) { return doof::Failure<std::string>{"Field \"explicitEntry\" expected boolean but got " + std::string(doof::json_type_name(_iterator_explicitEntry->second))}; }
        _field_explicitEntry = (_lenient ? doof::json_as_bool_lenient(_iterator_explicitEntry->second) : doof::json_as_bool(_iterator_explicitEntry->second));
    } else {
        _field_explicitEntry = false;
    }
    ...
    if (auto _iterator_resources = _object->find("resources"); _iterator_resources != _object->end()) {
        ...
        _field_resources = [&]() { const auto* _array = doof::json_as_array(_iterator_resources->second); auto _values = std::make_shared<std::vector<std::shared_ptr<::app_src_package_manifest_::PackageResource>>>(); _values->reserve(_array->size()); for (const auto& _element : *_array) { _values->push_back(doof::json_decode_value(::app_src_package_manifest_::PackageResource::fromJsonValue(_element, _lenient))); } return _values; }();
    } else {
    ...
    if (auto _iterator_externalDependencies = _object->find("externalDependencies"); _iterator_externalDependencies != _object->end()) {
        ...
        _field_externalDependencies = [&]() { const auto* _array = doof::json_as_array(_iterator_externalDependencies->second); auto _values = std::make_shared<std::vector<std::shared_ptr<::app_src_package_manifest_::ExternalDependency>>>(); _values->reserve(_array->size()); for (const auto& _element : *_array) { _values->push_back(doof::json_decode_value(::app_src_package_manifest_::ExternalDependency::fromJsonValue(_element, _lenient))); } return _values; }();
    } else {
    ...
    auto _field_nativeBuild = doof::json_decode_value(::app_src_package_manifest_::NativeBuildPlan::fromJsonValue(_iterator_nativeBuild->second, _lenient));
