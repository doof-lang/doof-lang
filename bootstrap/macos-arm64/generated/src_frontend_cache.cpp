...
doof::Result<std::shared_ptr<FrontendCacheState>, std::string> FrontendCacheState::fromJsonValue(const doof::JsonValue& _json, bool _lenient) {
    ...
    if (auto _iterator_probes = _object->find("probes"); _iterator_probes != _object->end()) {
        ...
        _field_probes = [&]() { const auto* _array = doof::json_as_array(_iterator_probes->second); auto _values = std::make_shared<std::vector<std::shared_ptr<FrontendSourceProbe>>>(); _values->reserve(_array->size()); for (const auto& _element : *_array) { _values->push_back(doof::success_value(FrontendSourceProbe::fromJsonValue(_element, _lenient))); } return _values; }();
    } else {
    ...
    if (auto _iterator_modules = _object->find("modules"); _iterator_modules != _object->end()) {
        ...
        _field_modules = [&]() { const auto* _array = doof::json_as_array(_iterator_modules->second); auto _values = std::make_shared<std::vector<std::shared_ptr<FrontendModuleOutput>>>(); _values->reserve(_array->size()); for (const auto& _element : *_array) { _values->push_back(doof::success_value(FrontendModuleOutput::fromJsonValue(_element, _lenient))); } return _values; }();
    } else {
