...
void ensureDirectory(const std::string& path) {
    ...
    static_cast<void>([&]() -> std::monostate { auto _try_value = ::doof_fs::mkdir(path); if (doof::is_failure(_try_value)) doof::panic_at("src/ios-app-driver", 63, std::string("try! failed"));  return {}; }());
}
...
void removeTree(const std::string& path) {
    ...
    static_cast<void>([&]() -> std::monostate { auto _try_value = ::doof_fs::remove(path); if (doof::is_failure(_try_value)) doof::panic_at("src/ios-app-driver", 83, std::string("try! failed"));  return {}; }());
}
...
doof::Result<void, std::string> configureIOSNativeBuild(const std::string& outputDirectory, const std::shared_ptr<::app_src_ios_app_::IOSAppConfig>& config, const std::string& destination, const std::shared_ptr<::app_src_package_manifest_::NativeBuildPlan>& native) {
    ...
    static_cast<void>([&]() -> std::monostate { auto _try_value = ::doof_fs::writeText(outputPath(outputDirectory, std::string("Info.plist")), ::app_src_ios_app_::renderIOSInfoPlist(config)); if (doof::is_failure(_try_value)) doof::panic_at("src/ios-app-driver", 102, std::string("try! failed"));  return {}; }());
    static_cast<void>([&]() -> std::monostate { auto _try_value = ::doof_fs::writeText(outputPath(outputDirectory, std::string("ios-main.mm")), ::app_src_ios_app_::renderIOSMainSource(config->executableName)); if (doof::is_failure(_try_value)) doof::panic_at("src/ios-app-driver", 103, std::string("try! failed"));  return {}; }());
...
std::string globBaseDirectory(const std::string& pattern) {
    ...
    for (int32_t index = 0; index < static_cast<int32_t>(pattern.size()); ++index) {
        if (doof::string_substring(pattern, index, (index + 1)) == std::string("*")) {
            static_cast<void>((wildcard = index));
            break;
        }
    }
...
doof::Result<void, std::string> copyIOSResources(const std::shared_ptr<::app_src_ios_app_::IOSAppConfig>& config, const std::string& appPath) {
    ...
    for (const auto& resource : *_iterable_18) {
        ...
        if (doof::string_contains(resource->sourcePath, std::string("*"))) {
            ...
            static_cast<void>((baseDirectory = resource->sourcePath));
        } else if (::doof_fs::exists(resource->sourcePath)) {
            files->push_back(resource->sourcePath);
            static_cast<void>((baseDirectory = parentPath(resource->sourcePath)));
        }
...
doof::Result<void, std::string> compileIOSIcon(const std::shared_ptr<::app_src_ios_app_::IOSAppConfig>& config, const std::string& appPath, const std::string& destination, const std::string& buildDirectory) {
    ...
    static_cast<void>([&]() -> std::monostate { auto _try_value = ::doof_fs::writeText(outputPath(iconSetPath, std::string("Contents.json")), (::app_src_ios_app_::renderIOSIconSetContents() + std::string("\n"))); if (doof::is_failure(_try_value)) doof::panic_at("src/ios-app-driver", 187, std::string("try! failed"));  return {}; }());
    ...
    if (::doof_fs::exists(partialPlistPath)) {
        static_cast<void>([&]() -> std::monostate { auto _try_value = ::doof_fs::remove(partialPlistPath); if (doof::is_failure(_try_value)) doof::panic_at("src/ios-app-driver", 216, std::string("try! failed"));  return {}; }());
    }
    return doof::Success<void>{};
}
doof::Result<std::string, std::string> assembleIOSApp(const std::string& buildDirectory, const std::string& executablePath, const std::shared_ptr<::app_src_ios_app_::IOSAppConfig>& config, const std::string& destination) {
    ...
    if (::doof_fs::exists(infoPlistPath)) {
        ...
        static_cast<void>([&]() -> std::monostate { auto _try_value = ::doof_fs::writeText(outputPath(appPath, std::string("Info.plist")), ::app_src_ios_app_::renderIOSInfoPlist(config)); if (doof::is_failure(_try_value)) doof::panic_at("src/ios-app-driver", 241, std::string("try! failed"));  return {}; }());
    }
...
doof::Result<void, std::string> signAndArchiveIOSApp(const std::string& appPath, const std::string& archivePath, const std::string& bundleId, const std::shared_ptr<::app_src_ios_app_::IOSPackageConfig>& config, const std::string& buildDirectory) {
        ...
        if (std::holds_alternative<doof::Success<std::string>>(_case_subject)) {
            const auto& success = std::get<doof::Success<std::string>>(_case_subject);
            {
                ...
                if (std::holds_alternative<doof::Success<int32_t>>(_case_subject)) {
                    ...
                    static_cast<void>((keychainGroupCount = parsedCount.value));
            }
    ...
    if (::doof_fs::exists(archivePath)) {
        static_cast<void>([&]() -> std::monostate { auto _try_value = ::doof_fs::remove(archivePath); if (doof::is_failure(_try_value)) doof::panic_at("src/ios-app-driver", 340, std::string("try! failed"));  return {}; }());
    }
